...
class SummerizationTrainer(Seq2SeqTrainer):
    ...
    def _maybe_log_save_evaluate(self, tr_loss, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        ...
        if self.control.should_evaluate:
            ...
            for metric_name, metric_value in metrics.items():
                ...
                if best_metric_key not in self.best_metrics or metric_value > self.best_metrics[best_metric_key]:
                    self.best_metrics[best_metric_key] = metric_value
                    if metric_name == f"eval_{self.args.metric_for_best_model}":
